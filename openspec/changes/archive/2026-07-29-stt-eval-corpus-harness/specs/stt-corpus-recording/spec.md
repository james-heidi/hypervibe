...
While recording is enabled, each completed push-to-talk utterance that reaches the ASR engine SHALL persist to a local corpus directory: (a) the exact mono PCM audio submitted to the engine (post gain-boost), as a WAV file; (b) the raw ASR transcript; (c) the polished transcript actually typed (when polish ran); (d) metadata including timestamp, engine id, sample rate, sample count, and decode wall-clock time.

#### Scenario: Utterance recorded
- **WHEN** recording is enabled and the user completes a dictation that produces text
- **THEN** a WAV file and a metadata record for that utterance exist in the corpus directory, and the metadata's raw transcript matches what the engine returned
